...
public class Validate extends Task {
	...
	private void validateAdminCert() throws ToolException {
		...
		boolean isAtLeastOneAdmin = ctx.getConfig().getClients().stream()
				.anyMatch(Config.Client::isAdmin);
		if (!isAtLeastOneAdmin) {
			throw new ToolException(
					"No client certificate was elected as admin certificate. If no admin certificate is present, the ES cluster cannot be used. " +
							"Please specify admin: true for at least one client certificate. In order to generate the certificates anyway, specify the -f flag.");
		}
	}
}
